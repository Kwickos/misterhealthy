...
describe("formatMenuOverview", () => {
  it("returns HTML with the week start date in the header", () => {
    const result = formatMenuOverview("fr", mockMenuData, "3 mars");
    expect(result).toContain("Menu semaine du 3 mars");
  });
  ...
  it("uses raw day key when no label is defined", () => {
    ...
    const result = formatMenuOverview("fr", menu, "3 mars");
    expect(result).toContain("<b>day.custom_day</b>");
  });
  ...
  it("uses English labels when locale is en", () => {
    const result = formatMenuOverview("en", mockMenuData, "March 3");
    expect(result).toContain("Menu for the week of March 3");
    expect(result).toContain("<b>Monday</b>");
    expect(result).toContain("<b>Tuesday</b>");
    expect(result).toContain("Lunch : Poulet rôti");
  });
});
...
describe("formatDayDetail", () => {
  it("returns HTML with the correct day label", () => {
    ...
    const result = formatDayDetail("fr", "lundi", dayMenu);
    expect(result).toContain("<b>Lundi</b>");
  });
  ...
  it("uses raw day key for unknown days", () => {
    const dayMenu: DayMenu = { dejeuner: mockMeal };
    const result = formatDayDetail("fr", "jour_special", dayMenu);
    expect(result).toContain("<b>day.jour_special</b>");
  });

  it("handles a day with a single meal", () => {
    const dayMenu: DayMenu = { petit_dej: mockMealSimple };
    const result = formatDayDetail("fr", "samedi", dayMenu);
...
describe("formatRecipeStep", () => {
  it("shows correct step number and total", () => {
    const result = formatRecipeStep("fr", mockMeal, 0);
  ...
  it("shows the correct step content for step 1", () => {
    const result = formatRecipeStep("fr", mockMeal, 1);
  ...
  it("shows the correct step content for the last step", () => {
    const result = formatRecipeStep("fr", mockMeal, 2);
  ...
  it("shows English step label for en locale", () => {
    const result = formatRecipeStep("en", mockMeal, 0);
    expect(result).toContain("<b>Step 1/3</b>");
  });
});

describe("formatIngredients", () => {
  it("includes the meal name in the header", () => {
    const result = formatIngredients("fr", mockMeal);
  ...
  it("handles a meal with a single ingredient", () => {
    ...
    const result = formatIngredients("fr", meal);
  ...
  it("uses English header for en locale", () => {
    const result = formatIngredients("en", mockMeal);
    expect(result).toContain("<b>Ingredients — Poulet rôti</b>");
  });
});
...
describe("formatBatchCooking", () => {
  it("returns empty string when no batch_cooking", () => {
    const menu: MenuData = { days: {} };
    expect(formatBatchCooking("fr", menu)).toBe("");
  });
  ...
  it("formats batch cooking day in the header", () => {
    ...
    const result = formatBatchCooking("fr", menu);
